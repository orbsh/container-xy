...
if not ($flag | path exists) {
    ...
    # Create warehouse if OSS credentials provided
    let ak = $env.OSS_ACCESS_KEY? | default ""
    let sk = $env.OSS_SECRET_KEY? | default ""
    if ($ak | is-not-empty) and ($sk | is-not-empty) {
        let wh = $env.LAKEKEEPER_WAREHOUSE? | default "default"
            ...
            let r = do -i {
                ...
                http post --allow-errors -t application/json
                    http://localhost:8181/management/v1/warehouse
                    {
                        ...
                        "storage-profile": {
                            type: "s3",
                            bucket: ($env.OSS_BUCKET? | default ""),
                            region: ($env.OSS_REGION? | default ""),
                            flavor: "s3-compat",
                            endpoint: ($env.OSS_ENDPOINT? | default ""),
                            "path-style-access": false,
                            "sts-enabled": false,
                            "key-prefix": ($env.OSS_KEY_PREFIX? | default "")
                        }
                    }
                )
            }
        ...
        print "WARNING: OSS credentials not set, skipping warehouse creation."
    }
